...
function doGet(e) {
  // GitHub Pages bridge endpoint.
  if (e && e.parameter && e.parameter.bridge === '1') {
    return HtmlService.createHtmlOutputFromFile('ApiBridge')
      .setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL)
      .setTitle('Castrol API Bridge');
  }
  try {
    ensureOrderSheetMigration_();
...
/**
 * API dispatcher untuk frontend yang di-host di GitHub Pages.
 * Hanya method yang memang dipakai frontend yang boleh dipanggil.
 */
function apiDispatch(method, args) {
  var allowed = {
    deleteDataOutletBWSAdmin:1, deleteTambahCustomer:1, getBWSFormMasterAdmin:1,
    getCustomerByCustCode:1, getDataOutletBWSAdmin:1, getDetailOrderById:1,
    getEditableOrderById:1, getListCashback:1, getListProduk:1,
    getListSalesOptions:1, getMonitoringOutletData:1, getOutletKontrakData_BWS:1,
    getPencapaianSales:1, getPricelistData:1, getProdukDibeliOutletQuarter:1,
    getRiwayatOrder:1, getTambahCustomerMasterData:1, getTambahCustomerTableData:1,
    getWhatsAppUrlById:1, hapusOrderById:1, hapusOrderByIds:1, prosesLogin:1,
    prosesRegister:1, saveDataOutletBWSAdmin:1, simpanMultiOrder:1,
    simpanTambahCustomer:1, updateAccount:1, updateOrderById:1,
    updateStatusApprove:1, updateTambahCustomer:1
  };
  if (!allowed[method]) throw new Error('Method API tidak diizinkan: ' + method);
  args = Array.isArray(args) ? args : [];
  switch (method) {
    case 'deleteDataOutletBWSAdmin': return deleteDataOutletBWSAdmin.apply(null, args);
    case 'deleteTambahCustomer': return deleteTambahCustomer.apply(null, args);
    case 'getBWSFormMasterAdmin': return getBWSFormMasterAdmin.apply(null, args);
    case 'getCustomerByCustCode': return getCustomerByCustCode.apply(null, args);
    case 'getDataOutletBWSAdmin': return getDataOutletBWSAdmin.apply(null, args);
    case 'getDetailOrderById': return getDetailOrderById.apply(null, args);
    case 'getEditableOrderById': return getEditableOrderById.apply(null, args);
    case 'getListCashback': return getListCashback.apply(null, args);
    case 'getListProduk': return getListProduk.apply(null, args);
    case 'getListSalesOptions': return getListSalesOptions.apply(null, args);
    case 'getMonitoringOutletData': return getMonitoringOutletData.apply(null, args);
    case 'getOutletKontrakData_BWS': return getOutletKontrakData_BWS.apply(null, args);
    case 'getPencapaianSales': return getPencapaianSales.apply(null, args);
    case 'getPricelistData': return getPricelistData.apply(null, args);
    case 'getProdukDibeliOutletQuarter': return getProdukDibeliOutletQuarter.apply(null, args);
    case 'getRiwayatOrder': return getRiwayatOrder.apply(null, args);
    case 'getTambahCustomerMasterData': return getTambahCustomerMasterData.apply(null, args);
    case 'getTambahCustomerTableData': return getTambahCustomerTableData.apply(null, args);
    case 'getWhatsAppUrlById': return getWhatsAppUrlById.apply(null, args);
    case 'hapusOrderById': return hapusOrderById.apply(null, args);
    case 'hapusOrderByIds': return hapusOrderByIds.apply(null, args);
    case 'prosesLogin': return prosesLogin.apply(null, args);
    case 'prosesRegister': return prosesRegister.apply(null, args);
    case 'saveDataOutletBWSAdmin': return saveDataOutletBWSAdmin.apply(null, args);
    case 'simpanMultiOrder': return simpanMultiOrder.apply(null, args);
    case 'simpanTambahCustomer': return simpanTambahCustomer.apply(null, args);
    case 'updateAccount': return updateAccount.apply(null, args);
    case 'updateOrderById': return updateOrderById.apply(null, args);
    case 'updateStatusApprove': return updateStatusApprove.apply(null, args);
    case 'updateTambahCustomer': return updateTambahCustomer.apply(null, args);
    default: throw new Error('Method API tidak diizinkan: ' + method);
  }
}

function ensureOrderSheetMigration_() {
  try {
    var ss = SpreadsheetApp.openById(SS_ID);
    var po = ss.getSheetByName('PO OUTLET');
    var oldInput = ss.getSheetByName('INPUT');
    if (!po && oldInput) { oldInput.setName('PO OUTLET'); po = oldInput; }
    var orders = ss.getSheetByName('ORDERS');
    var oldDamen = ss.getSheetByName('DAMEN');
    if (!orders && oldDamen) { oldDamen.setName('ORDERS'); orders = oldDamen; }
    return { poOutlet: po, orders: orders };
  } catch(e) {
    Logger.log('ensureOrderSheetMigration_: '+e.message);
    return { poOutlet:null, orders:null };
  }
}
...
function _getCustomerContextByCustCode_(custCode) {
  var key = String(custCode || '').trim().toUpperCase();
  if (!key) return null;
  var ss = SpreadsheetApp.openById(SS_ID);
  var sh = ss.getSheetByName('OUTLET');
  if (!sh || sh.getLastRow() < 2) return null;
  var d = sh.getDataRange().getValues();
  var h = d[0].map(function(x){ return String(x == null ? '' : x).trim().toUpperCase(); });
  var cCust = h.indexOf('CUST.CODE'); if (cCust < 0) cCust = h.indexOf('CUST CODE');
  var cOutlet = h.indexOf('OUTLET');
  var cSales = h.indexOf('SALES'); if (cSales < 0) cSales = h.indexOf('DSR');
  var cChannel = h.indexOf('CHANNEL OUTLET'); if (cChannel < 0) cChannel = h.indexOf('CHANNEL');
  if (cCust < 0) return null;
  for (var i=1;i<d.length;i++) {
    if (String(d[i][cCust] == null ? '' : d[i][cCust]).trim().toUpperCase() === key) {
      return {
        custCode: String(d[i][cCust] || '').trim(),
        outlet: cOutlet >= 0 ? String(d[i][cOutlet] || '').trim() : '',
        sales: cSales >= 0 ? String(d[i][cSales] || '').trim() : '',
        channel: cChannel >= 0 ? String(d[i][cChannel] || '').trim() : ''
      };
    }
  }
  return null;
...
function prosesLogin(custCode) {
  try {
    var cleanCode = String(custCode || '').trim();
    if (!cleanCode) return { success:false, message:'Username / CUST.CODE wajib diisi.' };

    // Login sekarang hanya menggunakan username / CUST.CODE.
    // Admin juga tidak lagi membutuhkan password.
    if (cleanCode.toLowerCase() === 'admin') {
      return { success:true, custCode:'admin', sales:'Admin', outlet:'', channel:'', role:'admin' };
    }

    var ctx = _getCustomerContextByCustCode_(cleanCode);
    if (!ctx) return { success:false, message:'CUST.CODE tidak ditemukan pada sheet OUTLET.' };

    return { success:true, custCode:ctx.custCode, sales:ctx.sales, outlet:ctx.outlet, channel:ctx.channel, role:'customer' };
  } catch(e) {
...
function getCustomerByCustCode(custCode) {
  var ctx = _getCustomerContextByCustCode_(custCode);
  return ctx || { custCode:String(custCode||'').trim(), outlet:'', sales:'', channel:'' };
}
function getOutletsByCustCode(custCode) {
  var ctx = _getCustomerContextByCustCode_(custCode);
  return ctx ? [ctx] : [];
}
...
function refreshDerivedForNewOrder_(sheetDamen, sheetInput, idInput, items, selectedCashback, formattedDate) {
  try {
    ...
    // ORDERS baru bukan struktur DAMEN. Jika kolom turunan DAMEN tidak ada,
    // jangan membuat/menulis kolom tambahan ke sheet ORDERS.
    if (headers.indexOf('VOLUME') < 0 || headers.indexOf('VALUE') < 0 || headers.indexOf('AK') < 0) return;
    ...
    // Update PO OUTLET kolom L untuk ID yang sama, bukan berdasarkan lastRow global.
...
function getMonitoringOutletData(salesName, usernameInput) { var admin=String(usernameInput||'').trim().toLowerCase()==='admin'; var custKey=String(usernameInput||'').trim().toUpperCase(); var k=admin?'MONITORING_ADMIN_CUST_V5':'MONITORING_CUST_V5_'+custKey; var c=_cacheGetJson(k); if(c) return c; var r=getMonitoringOutletData_UNCACHED(salesName,usernameInput); _cachePutJson(k,r,APP_CACHE_TTL); return r; }
...
function getDataOutletBWSAdmin(usernameInput) {
  try {
    _assertAdminBWS_(usernameInput);
    var sheet = _getDataOutletBWSSheet_();
    var headers = _getBWSHeaders_(sheet);
    return { success: true, headers: headers, rows: _getBWSRowsForClient_(sheet, headers) };
  } catch (e) {
...
function getPricelistData(forceRefresh) {
  try {
    ...
    var cacheKey = 'PRICELIST_DATA_V2';
    if (!forceRefresh) {
...
/**
 * Memastikan struktur sheet ORDERS memiliki minimal 40 kolom (AN)
...
function ensureDamenStructure_() {
  // ORDERS mempunyai struktur SENDIRI dan TIDAK mengikuti struktur sheet DAMEN.
  // Header resmi ORDERS:
  // A TIMESTAMP | B TANGGAL | C NO PO | D ID DAMEN | E ID INPUT |
  // F SALES | G OUTLET | H SKU PRODUK | I QTY | J HARGA SATUAN |
  // K TOTAL | L SATUAN | M CUST.CODE | N ORDER
  var ss = SpreadsheetApp.openById(SS_ID);
  var sh = ss.getSheetByName("ORDERS");
  if (!sh) {
    sh = ss.insertSheet("ORDERS");
    sh.getRange(1,1,1,14).setValues([[
      "TIMESTAMP","TANGGAL","NO PO","ID DAMEN","ID INPUT","SALES","OUTLET",
      "SKU PRODUK","QTY","HARGA SATUAN","TOTAL","SATUAN","CUST.CODE","ORDER"
    ]]);
...
function _ordersHeaderIndex_(headers, name, fallback) {
  var h = headers.map(function(x){ return String(x == null ? '' : x).trim().toUpperCase(); });
  var i = h.indexOf(String(name || '').trim().toUpperCase());
  return i >= 0 ? i : fallback;
}

function _getOrdersProductsById_(sheetOrders, idInput) {
  var out = [];
  if (!sheetOrders || sheetOrders.getLastRow() < 2) return out;
  var data = sheetOrders.getDataRange().getValues();
  var h = data[0].map(function(x){ return String(x == null ? '' : x).trim().toUpperCase(); });
  var iId = _ordersHeaderIndex_(h, 'ID INPUT', 4);
  var iSku = _ordersHeaderIndex_(h, 'SKU PRODUK', 7);
  var iQty = _ordersHeaderIndex_(h, 'QTY', 8);
  var iUnit = _ordersHeaderIndex_(h, 'SATUAN', 11);
  var iOrder = _ordersHeaderIndex_(h, 'ORDER', 13);
  for (var r = 1; r < data.length; r++) {
    if (String(data[r][iId] == null ? '' : data[r][iId]).trim() !== String(idInput || '').trim()) continue;
    var orderText = String(data[r][iOrder] == null ? '' : data[r][iOrder]).trim();
    if (!orderText) {
      var sku = String(data[r][iSku] == null ? '' : data[r][iSku]).trim();
      var qty = data[r][iQty] == null ? '' : data[r][iQty];
      var unit = String(data[r][iUnit] == null ? '' : data[r][iUnit]).trim();
      orderText = sku + (qty !== '' ? ' - ' + qty + ' ' + unit : '');
    }
    if (orderText) out.push(orderText);
  }
  return out;
}
...
function simpanMultiOrder(orderPayload) {
  try {
    // TAB INPUT: CUST.CODE, OUTLET, SALES dan CHANNEL selalu ditentukan dari login CUST.CODE.
    // Payload dari browser hanya membawa CUST.CODE login; OUTLET/CHANNEL diabaikan dan dihitung ulang dari OUTLET.

    var ss = SpreadsheetApp.openById(SS_ID);
    ensureOrderSheetMigration_();
    var items = orderPayload.items;

    // Identitas order sekarang sepenuhnya berasal dari CUST.CODE yang sedang login.
    var loginCustCode = String(orderPayload.custCode || '').trim();
    var customerCtx = _getCustomerContextByCustCode_(loginCustCode);
    if (!customerCtx) {
      return { success:false, message:'CUST.CODE tidak valid atau tidak ditemukan di sheet OUTLET.' };
    }
    orderPayload.custCode = customerCtx.custCode;
    orderPayload.outlet = customerCtx.outlet;
    orderPayload.sales = customerCtx.sales;
    orderPayload.channel = customerCtx.channel;
    orderPayload.tanggal = orderPayload.tanggal || Utilities.formatDate(new Date(), Session.getScriptTimeZone(), 'yyyy-MM-dd');
    orderPayload.payment = orderPayload.payment || 'CASH';
    orderPayload.disc = orderPayload.disc == null ? '0%' : orderPayload.disc;
    orderPayload.note = orderPayload.note || '-';
    orderPayload.cashback = orderPayload.cashback || '';
    ...
    var sheetInput = ss.getSheetByName("PO OUTLET");
    if (!sheetInput) {
      sheetInput = ss.insertSheet("PO OUTLET");
    ...
    var sheetOrders = ensureDamenStructure_();
    if (!sheetOrders) return { success: false, message: "Sheet ORDERS tidak ditemukan." };

    var ordersHeaders = sheetOrders.getRange(1, 1, 1, Math.max(14, sheetOrders.getLastColumn())).getValues()[0];
    var ordersHeaderText = ordersHeaders.map(function(h){ return String(h == null ? '' : h).trim().toUpperCase(); });
    var ordersWidth = Math.max(14, ordersHeaders.length);
    if (ordersWidth < 14) ordersWidth = 14;
    if (sheetOrders.getMaxColumns() < ordersWidth) {
      sheetOrders.insertColumnsAfter(sheetOrders.getMaxColumns(), ordersWidth - sheetOrders.getMaxColumns());
    }

    var ordersRowsToWrite = [];
    var timestampNow = new Date();
    var orderDate = orderPayload.tanggal ? new Date(String(orderPayload.tanggal) + 'T00:00:00') : new Date();
    if (isNaN(orderDate.getTime())) orderDate = new Date();
    var noPoValue = orderPayload.noPo == null ? '' : String(orderPayload.noPo).trim();
    ...
    for (var j = 0; j < items.length; j++) {
      ...
      var row = new Array(ordersWidth).fill('');

      function setOrdersField_(name, value) {
        var idx = ordersHeaderText.indexOf(String(name).trim().toUpperCase());
        if (idx >= 0) row[idx] = value;
      }

      // Pemetaan ORDERS BERDASARKAN HEADER ORDERS, bukan posisi/kolom DAMEN.
      setOrdersField_('TIMESTAMP', timestampNow);
      setOrdersField_('TANGGAL', orderDate);
      setOrdersField_('NO PO', noPoValue);
      setOrdersField_('ID DAMEN', idDamen);
      setOrdersField_('ID INPUT', idInput);
      setOrdersField_('SALES', salesName);
      setOrdersField_('OUTLET', orderPayload.outlet);
      setOrdersField_('SKU PRODUK', items[j].resolvedKode);
      setOrdersField_('QTY', items[j].qty);
      setOrdersField_('HARGA SATUAN', Number(items[j].harga) || 0);
      setOrdersField_('TOTAL', Number(items[j].netValue) || 0);
      setOrdersField_('SATUAN', items[j].satuanOpt || items[j].satuan || 'PCS');
      setOrdersField_('CUST.CODE', orderPayload.custCode);
      setOrdersField_('ORDER', trimSkuText);
      setOrdersField_('DISC', discValue); // hanya diisi jika header DISC memang ada

      ordersRowsToWrite.push(row);
    }

    if (ordersRowsToWrite.length) {
      var firstOrdersRow = sheetOrders.getLastRow() + 1;
      sheetOrders.getRange(firstOrdersRow, 1, ordersRowsToWrite.length, ordersWidth).setValues(ordersRowsToWrite);

      var discIdx = ordersHeaderText.indexOf('DISC');
      if (discIdx >= 0) sheetOrders.getRange(firstOrdersRow, discIdx + 1, ordersRowsToWrite.length, 1).setNumberFormat('0%');
      var tsIdx = ordersHeaderText.indexOf('TIMESTAMP');
      if (tsIdx >= 0) sheetOrders.getRange(firstOrdersRow, tsIdx + 1, ordersRowsToWrite.length, 1).setNumberFormat('dd/MM/yyyy HH:mm:ss');
      var dateIdx = ordersHeaderText.indexOf('TANGGAL');
      if (dateIdx >= 0) sheetOrders.getRange(firstOrdersRow, dateIdx + 1, ordersRowsToWrite.length, 1).setNumberFormat('dd/MM/yyyy');
      var priceIdx = ordersHeaderText.indexOf('HARGA SATUAN');
      if (priceIdx >= 0) sheetOrders.getRange(firstOrdersRow, priceIdx + 1, ordersRowsToWrite.length, 1).setNumberFormat('#,##0.00');
      var totalIdx = ordersHeaderText.indexOf('TOTAL');
      if (totalIdx >= 0) sheetOrders.getRange(firstOrdersRow, totalIdx + 1, ordersRowsToWrite.length, 1).setNumberFormat('#,##0.00');
    }

    // ORDERS tidak menjalankan turunan kolom DAMEN (AL/AM/AO). PO OUTLET sudah menyimpan VALUE sendiri.
    clearAppDataCaches();
  
    return { success: true, message: "Order berhasil disimpan!" };
  } catch(e) {
...
function getEditableOrderById(idInput,salesName,usernameInput){
  try {
    var ss=SpreadsheetApp.openById(SS_ID), sh=ss.getSheetByName('PO OUTLET');
    if(!sh||sh.getLastRow()<2)return{success:false,message:'Order tidak ditemukan.'};
    var d=sh.getDataRange().getValues(), hd=d[0].map(function(x){return String(x).trim().toUpperCase();});
    var idx=function(n,f){var x=hd.indexOf(n);return x>=0?x:f;};
    var cId=idx('ID INPUT',0),cOutlet=idx('OUTLET',1),cDate=idx('DATE',2),cPay=idx('PAYMENT',5),cDisc=idx('DISC',6),cCust=idx('CUST.CODE',7),cChan=idx('CHANNEL',8),cNote=idx('NOTE ORDER',9),cStatus=idx('STATUS APPROVE',12),cCb=idx('CASHBACK',13);
    var target=String(idInput||'').trim(),admin=String(usernameInput||'').trim().toLowerCase()==='admin',salesKey=String(salesName||'').trim().toUpperCase(),found=null;
    for(var r=1;r<d.length;r++)if(String(d[r][cId]||'').trim()===target){found=d[r];break;}
    if(!found)return{success:false,message:'Order dengan ID INPUT '+target+' tidak ditemukan.'};
    var owner=String(found[cCust]||'').trim();
    if(!admin&&owner.toUpperCase()!==salesKey)return{success:false,message:'Anda hanya dapat mengedit order milik CUST.CODE Anda sendiri.'};

    var products=[];
    var dm=ss.getSheetByName('ORDERS');
    if(dm&&dm.getLastRow()>=2){
      var dd=dm.getDataRange().getValues(), dh=dd[0].map(function(x){return String(x).trim().toUpperCase();});
      var iId=_ordersHeaderIndex_(dh,'ID INPUT',4), iSku=_ordersHeaderIndex_(dh,'SKU PRODUK',7), iQty=_ordersHeaderIndex_(dh,'QTY',8), iUnit=_ordersHeaderIndex_(dh,'SATUAN',11), iHarga=_ordersHeaderIndex_(dh,'HARGA SATUAN',9);
      var pm=_getEditableProductMap_();
      for(var j=1;j<dd.length;j++){
        if(String(dd[j][iId]||'').trim()!==target)continue;
        var kode=String(dd[j][iSku]||'').trim(), qty=Number(dd[j][iQty])||0, unit=String(dd[j][iUnit]||'PCS').trim(), inf=_resolveEditableProduct_(pm,kode,kode)||_resolveEditableProductDirect_('',kode)||{};
        var nm=inf.nama||kode;
        products.push({kode:kode,produk:nm,qty:qty,satuan:unit,pack:Number(inf.pack)||1,harga:Number(dd[j][iHarga])||Number(inf.harga)||0,liter:Number(inf.liter)||1});
      }
    }
    var dateStr='';
    if(found[cDate] instanceof Date)dateStr=Utilities.formatDate(found[cDate],Session.getScriptTimeZone(),'yyyy-MM-dd');
    else{try{var qd=new Date(String(found[cDate]||''));if(!isNaN(qd.getTime()))dateStr=Utilities.formatDate(qd,Session.getScriptTimeZone(),'yyyy-MM-dd');}catch(e){}}
    return{success:true,order:{idInput:target,outlet:found[cOutlet]||'',tanggal:dateStr,payment:found[cPay]||'',disc:Number(found[cDisc])||0,custCode:found[cCust]||'',channel:found[cChan]||'',note:found[cNote]||'',cashback:found[cCb]||'',status:found[cStatus]||'PENDING',sales:owner,products:products}};
  }catch(e){return{success:false,message:'Gagal memuat order: '+e.message};}
}
function updateOrderById(idInput,payload,salesName,usernameInput){
  try{
    payload=payload||{};
    var ss=SpreadsheetApp.openById(SS_ID), sh=ss.getSheetByName('PO OUTLET'), dm=ensureDamenStructure_();
    if(!sh||!dm)return{success:false,message:'Sheet PO OUTLET atau ORDERS tidak ditemukan.'};
    var d=sh.getDataRange().getValues(),hd=d[0].map(function(x){return String(x).trim().toUpperCase();}),idx=function(n,f){var x=hd.indexOf(n);return x>=0?x:f;};
    var cId=idx('ID INPUT',0),target=String(idInput||'').trim(),admin=String(usernameInput||'').trim().toLowerCase()==='admin',salesKey=String(salesName||'').trim().toUpperCase(),rn=-1;
    for(var r=1;r<d.length;r++)if(String(d[r][cId]||'').trim()===target){rn=r+1;break;}
    if(rn<0)return{success:false,message:'Order tidak ditemukan.'};
    var old=d[rn-1],cCust=idx('CUST.CODE',7),owner=String(old[cCust]||'').trim();
    if(!admin&&owner.toUpperCase()!==salesKey)return{success:false,message:'Anda tidak berhak mengedit order milik CUST.CODE ini.'};

    var customerCtx=_getCustomerContextByCustCode_(payload.custCode || salesName);
    if(!admin && !customerCtx)return{success:false,message:'CUST.CODE tidak valid.'};
    if(customerCtx){payload.custCode=customerCtx.custCode;payload.outlet=customerCtx.outlet;payload.channel=customerCtx.channel;payload.sales=customerCtx.sales;}

    var items=Array.isArray(payload.items)?payload.items:[];
    if(!items.length)return{success:false,message:'Minimal harus ada 1 produk.'};
    var pm=_getEditableProductMap_(),prep=[],tv=0,tn=0;
    var editDiscRaw=payload.disc==null?'':String(payload.disc).trim().replace(',','.');
    var editDiscValue=0;
    if(editDiscRaw!==''){var hasPct=editDiscRaw.indexOf('%')!==-1,num=parseFloat(editDiscRaw.replace('%','').trim());if(!isNaN(num))editDiscValue=hasPct||Math.abs(num)>1?num/100:num;}

    for(var i=0;i<items.length;i++){
      var nm=String(items[i].produk||'').trim(),code=String(items[i].kode||'').trim();
      var inf=_resolveEditableProduct_(pm,nm,code)||_resolveEditableProductDirect_(nm,code);
      if(!inf && nm)inf={nama:nm,kode:code,liter:Number(items[i].liter)||1,harga:Number(items[i].harga)||0,pack:Number(items[i].pack)||1};
      if(!inf)return{success:false,message:'Produk tidak valid: '+nm+(code?' (Kode: '+code+')':'')};
      var qty=Number(items[i].qty)||0,unit=String(items[i].satuan||'PCS').trim().toUpperCase();
      if(qty<=0)return{success:false,message:'Qty produk harus lebih dari 0.'};
      var pcs=unit==='DUS'?qty*(Number(inf.pack)||1):qty;
      var unitHarga=Number(items[i].harga)||Number(inf.harga)||0;
      var vol=pcs*(Number(inf.liter)||1),gross=pcs*unitHarga,net=gross*(1-editDiscValue);if(net<0)net=0;
      tv+=vol;tn+=net;
      prep.push({produk:nm,kode:inf.kode||code,qty:qty,satuan:unit,harga:unitHarga,netValue:net});
    }

    // Update PO OUTLET tetap menggunakan struktur PO OUTLET.
    var dt=payload.tanggal?new Date(String(payload.tanggal)+'T00:00:00'):new Date();if(isNaN(dt.getTime()))dt=new Date();
    var tz=Session.getScriptTimeZone(),fd=Utilities.formatDate(dt,tz,'d MMMM yyyy'),mn=Utilities.formatDate(dt,tz,'MMMM'),yr=dt.getFullYear();
    var set=function(n,f,v){var x=idx(n,f);if(x>=0)old[x]=v;};
    set('OUTLET',1,payload.outlet||'');set('DATE',2,fd);set('MONTH',3,mn+'-'+String(yr).slice(-2));set('PAYMENT',5,payload.payment||'');set('DISC',6,editDiscValue);set('CUST.CODE',7,payload.custCode||'');set('CHANNEL',8,payload.channel||'');set('NOTE ORDER',9,payload.note||'');set('VOLUME',10,tv);set('VALUE',11,tn);set('CASHBACK',13,getCanonicalCashback_(payload.cashback));
    sh.getRange(rn,1,1,old.length).setValues([old]);

    // Hapus semua baris ORDERS untuk ID INPUT lalu tulis ulang dengan HEADER ORDERS.
    var od=dm.getDataRange().getValues(), oh=od[0].map(function(x){return String(x).trim().toUpperCase();}), oiId=_ordersHeaderIndex_(oh,'ID INPUT',4);
    for(var z=od.length-1;z>=1;z--)if(String(od[z][oiId]||'').trim()===target)dm.deleteRow(z+1);

    var width=Math.max(14,dm.getLastColumn()), rows=[];
    if(dm.getMaxColumns()<width)dm.insertColumnsAfter(dm.getMaxColumns(),width-dm.getMaxColumns());
    var timestampNow=new Date(),noPoValue=payload.noPo==null?'':String(payload.noPo).trim();
    for(var j=0;j<prep.length;j++){
      var row=new Array(width).fill('');
      var setO=function(n,v){var q=oh.indexOf(String(n).trim().toUpperCase());if(q>=0)row[q]=v;};
      var idDamen='DAM-EDIT-'+Utilities.getUuid().replace(/-/g,'').substring(0,12);
      var orderText=prep[j].produk+' - '+prep[j].qty+' '+prep[j].satuan;
      setO('TIMESTAMP',timestampNow);setO('TANGGAL',dt);setO('NO PO',noPoValue);setO('ID DAMEN',idDamen);setO('ID INPUT',target);setO('SALES',payload.sales||salesName);setO('OUTLET',payload.outlet||'');setO('SKU PRODUK',prep[j].kode);setO('QTY',prep[j].qty);setO('HARGA SATUAN',prep[j].harga);setO('TOTAL',prep[j].netValue);setO('SATUAN',prep[j].satuan);setO('CUST.CODE',payload.custCode||'');setO('ORDER',orderText);setO('DISC',editDiscValue);rows.push(row);
    }
    if(rows.length){var st=dm.getLastRow()+1;dm.getRange(st,1,rows.length,width).setValues(rows);var di=oh.indexOf('DISC');if(di>=0)dm.getRange(st,di+1,rows.length,1).setNumberFormat('0%');}
    clearAppDataCaches();
    return{success:true,message:'Order '+target+' berhasil diperbarui.'};
  }catch(e){return{success:false,message:'Gagal memperbarui order: '+e.message};}
}
function getRiwayatOrder_UNCACHED(custCode, usernameInput) {
  try {
    var ss = SpreadsheetApp.openById(SS_ID);
    ...
    var data = sheet.getDataRange().getValues();
    var headers = data[0].map(function(h){return String(h == null ? '' : h).trim().toUpperCase();});
    var idx=function(n,f){var x=headers.indexOf(n);return x>=0?x:f;};
    var colId=idx('ID INPUT',0), colOutlet=idx('OUTLET',1), colDate=idx('DATE',2), colCust=idx('CUST.CODE',7), colVol=idx('VOLUME',10), colVal=idx('VALUE',11), colStatus=idx('STATUS APPROVE',12);
    var isAdmin=String(usernameInput||'').trim().toLowerCase()==='admin';
    var custKey=String(custCode||'').trim().toUpperCase();
    var ordersSheet=ss.getSheetByName('ORDERS');
    var productsById={};
    if(ordersSheet && ordersSheet.getLastRow()>=2){
      var od=ordersSheet.getDataRange().getValues();
      var oh=od[0].map(function(h){return String(h==null?'':h).trim().toUpperCase();});
      var oi=oh.indexOf('ID INPUT'); if(oi<0)oi=0;
      var op=oh.indexOf('ORDER'); if(op<0)op=13;
      for(var r=1;r<od.length;r++){var id=String(od[r][oi]||'').trim();var prod=String(od[r][op]||'').trim();if(id&&prod){if(!productsById[id])productsById[id]=[];productsById[id].push(prod);}}
    }
    var result=[];
    for(var j=1;j<data.length;j++){
      var rowCust=String(data[j][colCust]||'').trim();
      if(!isAdmin && rowCust.toUpperCase()!==custKey) continue;
      var idValue=String(data[j][colId]||'').trim();
      var products=productsById[idValue]?productsById[idValue].slice():[];
      var dt=data[j][colDate];
      var tanggal='-';
      if(dt){try{tanggal=Utilities.formatDate(new Date(dt),Session.getScriptTimeZone(),'yyyy-MM-dd');}catch(e){}}
      result.push({id:idValue||'-',outlet:data[j][colOutlet]||'-',custCode:rowCust||'-',tanggal:tanggal,volume:Number(data[j][colVol]||0).toFixed(1),value:Number(data[j][colVal]||0),status:data[j][colStatus]||'PENDING',dsr:rowCust||'-',products:products});
    }
    return result.reverse();
  } catch(e){ Logger.log('Error getRiwayatOrder: '+e.message); return []; }
}
...
function getPencapaianSales_UNCACHED(salesName, usernameInput, selectedPeriod) {
  try {
    ...
    var sheetDamen=ss.getSheetByName('ORDERS');
    if (sheetDamen && sheetDamen.getLastRow() >= 2) {
      ...
      // PRODUK FOKUS menggunakan kolom AE / header SEGMENT PB di sheet ORDERS.
...
/**
 * Menentukan STATUS APPROVE pada sheet PO OUTLET berdasarkan ID INPUT.
...
function updateStatusApprove(idInput, status, usernameInput) {
  try {
    ...
    var lastRow = sheet.getLastRow();
    if (lastRow < 2) {
      return { success: false, message: "Belum ada data pada sheet PO OUTLET." };
    }
    ...
    for (var i = 1; i < data.length; i++) {
      if (String(data[i][colId] == null ? "" : data[i][colId]).trim() === cleanId) {
        ...
        // ORDERS baru tidak mempunyai kolom STATUS APPROVE.
        // Jangan membuat/mengisi kolom tambahan pada ORDERS hanya untuk status.
        var damenUpdated = 0;
        var sheetDamen = ss.getSheetByName("ORDERS");
        if (sheetDamen && sheetDamen.getLastRow() >= 2) {
          var ordersHdrStatus = sheetDamen.getRange(1,1,1,sheetDamen.getLastColumn()).getValues()[0].map(function(h){return String(h == null ? '' : h).trim().toUpperCase();});
          var statusColOrders = ordersHdrStatus.indexOf('STATUS APPROVE');
          var idColOrders = ordersHdrStatus.indexOf('ID INPUT');
          if (statusColOrders >= 0 && idColOrders >= 0) {
            var lastDamenRow = sheetDamen.getLastRow();
            var damenIds = sheetDamen.getRange(2, idColOrders + 1, lastDamenRow - 1, 1).getValues();
            var statusValues = sheetDamen.getRange(2, statusColOrders + 1, lastDamenRow - 1, 1).getValues();
            for (var d = 0; d < damenIds.length; d++) {
              if (String(damenIds[d][0] == null ? '' : damenIds[d][0]).trim() === cleanId) { statusValues[d][0] = cleanStatus; damenUpdated++; }
            }
            if (damenUpdated > 0) sheetDamen.getRange(2, statusColOrders + 1, statusValues.length, 1).setValues(statusValues);
          }
        }
        ...
        return {
          ...
          message: "Status " + cleanStatus + " berhasil disimpan untuk " + cleanId + ". PO OUTLET dan " + damenUpdated + " baris ORDERS telah diperbarui."
        };
      }
    }
...
function getDetailOrderById(idInput) {
  try {
    ...
    // ============================================================
    // RINCIAN PRODUK: SHEET DAMEN berdasarkan ID PO OUTLET kolom A
    ...
    if (sheetDamen && sheetDamen.getLastRow() >= 2) {
      var damenData = sheetDamen.getDataRange().getValues();
      var dh = damenData[0].map(function(h){ return String(h == null ? '' : h).trim().toUpperCase(); });
      var did = _ordersHeaderIndex_(dh, 'ID INPUT', 4);
      var dop = _ordersHeaderIndex_(dh, 'ORDER', 13);
      for (var i = 1; i < damenData.length; i++) {
        var damenId = String(damenData[i][did] == null ? '' : damenData[i][did]).trim();
        if (damenId !== targetId) continue;
        var produk = String(damenData[i][dop] == null ? '' : damenData[i][dop]).trim();
        products.push({ no: products.length + 1, produk: produk || '-' });
      }
    }
...
function _deleteRowsByIdsFast_(sheet, idSet) {
  ...
  // Cari kolom ID INPUT dari header masing-masing sheet.
  // PO OUTLET umumnya A, sedangkan ORDERS pada struktur baru adalah E.
  var header = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0].map(function(h){ return String(h == null ? '' : h).trim().toUpperCase(); });
  var idCol = header.indexOf('ID INPUT');
  if (idCol < 0) idCol = 0;
  var values = sheet.getRange(2, idCol + 1, lastRow - 1, 1).getValues();
  var rows = [];
  ...
  try {
    ...
    if (!sheetOrders) {
      return {
        success: false,
        whatsappUrl: "",
        message: 'Sheet "ORDERS" tidak ditemukan.'
      };
    }
    ...
    if (!data || data.length < 2) {
      return {
        success: false,
        whatsappUrl: "",
        message: "Tidak ada data pada sheet ORDERS."
      };
    }

    // ============================================================
    // BACA HEADER ORDERS BERDASARKAN NAMA HEADER
    // ============================================================
    var headers = data[0].map(function(h) {
    ...
    function getCol(headerName, fallbackIndex) {
      var idx = headers.indexOf(String(headerName).toUpperCase());
      return idx >= 0 ? idx : fallbackIndex;
    }

    // ORDERS:
    // ID INPUT  -> header ID INPUT
    // OUTLET    -> header OUTLET
    // ORDER     -> header ORDER
    var colIdInput = getCol("ID INPUT", 4);
    var colOutlet  = getCol("OUTLET", 6);
    var colOrder   = getCol("ORDER", 13);
    ...
    if (!targetId) {
      return {
        success: false,
        whatsappUrl: "",
        message: "ID INPUT tidak ditemukan."
      };
    }
    ...
    // HANYA BARIS DENGAN ID INPUT YANG SAMA
    // ============================================================
    for (var i = 1; i < data.length; i++) {

      var currentId = String(
        data[i][colIdInput] == null ? "" : data[i][colIdInput]
      ).trim();
      ...
      // Ambil outlet dari ORDERS
      if (!outletName) {
        outletName = String(
          data[i][colOutlet] == null ? "" : data[i][colOutlet]
        ).trim();
      }

      // ========================================================
      // ISI WHATSAPP DIAMBIL DARI KOLOM "ORDER" ORDERS
      // ========================================================
      var orderValue = String(
        data[i][colOrder] == null ? "" : data[i][colOrder]
      ).trim();

      if (orderValue) {
        listItems.push("- " + orderValue);
      }
    }

    // ============================================================
    // JIKA ID INPUT TIDAK DITEMUKAN DI ORDERS
    // ============================================================
    if (!listItems.length) {
      return {
        success: false,
        whatsappUrl: "",
        message: "ID INPUT " + targetId + " tidak ditemukan pada ORDERS."
      };
    }
    ...
    return {
      success: true,
      whatsappUrl: url,
      message: "Data WhatsApp berhasil dibuat dari ORDERS."
    };

  } catch (error) {

    Logger.log(
      "Error getWhatsAppUrlById: " +
...
/**
 * Fungsi untuk menghitung dan mengisi Kolom AL pada sheet ORDERS:
 * Cocokkan Kolom AK sheet ORDERS dengan Kolom H sheet CASHBACK,
 * lalu kalikan dengan Kolom F sheet CASHBACK dan Kolom P sheet ORDERS.
 */
function rebuildDamenDerivedColumns() {
  try {
    ...
    hitungKolomAODamen();
    return { success:true, message:'Kolom AL, AM, AO DAMEN dan PO OUTLET kolom L berhasil dihitung ulang.' };
  } catch(e) {
...
/**
 * Mengisi Kolom AM pada sheet ORDERS dengan:
 * AM = Q - AL
 *
 * Kolom Q  = kolom ke-17
...
/**
 * Mengisi Kolom L pada sheet PO OUTLET dengan total Kolom AM pada sheet ORDERS
 * yang dikelompokkan berdasarkan ID INPUT (ORDERS kolom A).
 *
...
/**
 * Mengisi Kolom AO pada sheet ORDERS dengan Tahun berformat YYYY
...
function getOutletKontrakData_BWS_UNCACHED(salesName, usernameInput) {
  try {
    ...
    var sheetDamen = ss.getSheetByName("ORDERS");
    if (!sheetDamen) {
      return { success: false, error: 'Sheet "ORDERS" tidak ditemukan.' };
    }
...
function getMonitoringOutletData_UNCACHED(salesName, usernameInput) {
  try {
    ...
    var isAdmin = (usernameInput && String(usernameInput).trim().toLowerCase() === 'admin');
    var loginCustCode = String(usernameInput || '').trim().toUpperCase();
    ...
    if (sheetOutletMaster) {
      ...
      if (dataOM.length > 1) {
        ...
        var idxCustOM = hOM.indexOf("CUST.CODE") !== -1 ? hOM.indexOf("CUST.CODE") : hOM.indexOf("CUST CODE");
        if (idxCustOM === -1) idxCustOM = 2;
        ...
        for (var o = 1; o < dataOM.length; o++) {
          ...
          var outCustCode = String(dataOM[o][idxCustOM] == null ? "" : dataOM[o][idxCustOM]).trim().toUpperCase();
          ...
          if (outName) {
            ...
            if (isAdmin || (loginCustCode && outCustCode === loginCustCode)) {
    ...
    if (sheetTargetOutlet) {
      ...
      if (targetOutletData.length > 1) {
        ...
        for (var ti = 1; ti < targetOutletData.length; ti++) {
          ...
          if (!isAdmin && (!loginCustCode || tc.toUpperCase() !== loginCustCode)) continue;
    ...
    // 3. Ambil data actual Monitoring dari sheet DAMEN.
    // ORDERS hanya menjadi fallback untuk kompatibilitas.
    var sheetDamen = ss.getSheetByName("DAMEN") || ss.getSheetByName("ORDERS");
    if (!sheetDamen) return {};
    ...
    // Struktur DAMEN saat ini: C=OUTLET, D=CUST.CODE, J=DATE, P=VOLUME.
    // Header diprioritaskan agar tetap aman bila posisi kolom berubah.
    var cDate = hDamen.indexOf("DATE") !== -1 ? hDamen.indexOf("DATE") : 9;
    var cVolume = hDamen.indexOf("VOLUME") !== -1 ? hDamen.indexOf("VOLUME") : 15;
    ...
    for (var i = 1; i < dataDamen.length; i++) {
      if (!isAdmin) {
        var orderCustCode = cCust !== -1 ? String(dataDamen[i][cCust] == null ? "" : dataDamen[i][cCust]).trim().toUpperCase() : "";
        if (!loginCustCode || orderCustCode !== loginCustCode) continue;
      }
    ...
    Object.keys(activeQuarters).forEach(function(qKey) {
      ...
      assignedOutlets.forEach(function(outletName) {
        ...
        aggregated[qKey][outletName] = {
          ...
          m2: 0,
          m3: 0,
          pointYear: Number(String(qKey).split(" ").pop()) || 0,
          pointYTD: 0,
          pointVolumeYTD: 0,
          pointKartonYTD: 0,
          pointByCategory: []
    ...
    for (var i = 1; i < dataDamen.length; i++) {
      ...
      // Untuk login customer, seluruh perhitungan Monitoring hanya mengambil
      // transaksi DAMEN/ORDERS dengan CUST.CODE yang sama dengan login.
      // Admin tetap dapat melihat seluruh CUST.CODE.
      if (!isAdmin) {
        var monitoringCustCode = cCust !== -1
          ? String(dataDamen[i][cCust] == null ? "" : dataDamen[i][cCust]).trim().toUpperCase()
          : "";
        if (!loginCustCode || monitoringCustCode !== loginCustCode) continue;
      }

      // Customer: pencarian Monitoring MURNI berdasarkan CUST.CODE login.
      // Tidak lagi dibatasi oleh SALES/DSR, sehingga semua transaksi DAMEN/ORDERS
      // milik CUST.CODE login tetap dihitung walaupun nilai DSR berbeda.
      // Admin tetap dapat melihat seluruh transaksi.
      ...
      // ACTUAL MONITORING mengambil VOLUME langsung dari kolom VOLUME di DAMEN/ORDERS.
      var tglVal = dataDamen[i][cDate];
      ...
      if (isNaN(dObj.getTime())) continue;
        ...
        var year = dObj.getFullYear();
        var monthIndex = dObj.getMonth();
        // Sistem bulan dalam quartal tetap mengikuti kalender:
        // Q1 = Jan-Feb-Mar, Q2 = Apr-Mei-Jun, Q3 = Jul-Agu-Sep, Q4 = Okt-Nov-Des.
        // monthInQuarter selalu bernilai 1, 2, atau 3 dan menjadi sumber
        // perhitungan M1/M2/M3 dari transaksi DAMEN/ORDERS.
        var quarter = Math.floor(monthIndex / 3) + 1;
        ...
        // PERHITUNGAN KARTON tetap menggunakan rumus sebelumnya:
        // BP / MCO-4T = Volume DAMEN / 9.6
        // CAS / PCO    = Volume DAMEN / 12
        // Jika channel belum dikenali, jangan mengubah nilai Volume secara diam-diam.
        var channelForKarton = String(channelVal || "").trim().toUpperCase();
        var volumeInCtn = rawVolume;
        if (channelForKarton === "BP" || channelForKarton === "MCO-4T") {
          volumeInCtn = rawVolume / 9.6;
        } else if (channelForKarton === "CAS" || channelForKarton === "PCO") {
        ...
        if (!aggregated[quarterKey][outletName]) {
          ...
          aggregated[quarterKey][outletName] = {
            ...
            m2: 0,
            m3: 0,
            pointYear: Number(String(quarterKey).split(" ").pop()) || 0,
            pointYTD: 0,
            pointVolumeYTD: 0,
            pointKartonYTD: 0,
            pointByCategory: []
    ...
    // ================================================================
    // 4. POINT TRIP BERDASARKAN TAHUN
    // Sumber:
    // - DAMEN: DATE (J), VOLUME (P), KATEGORI TRIP (AN), CUST.CODE (D)
    // - DATA TRIP BWS: kategori -> point rate
    // - Channel: TARGET OUTLET per outlet + quarter + CUST.CODE, fallback OUTLET
    // Rumus: BP/MCO-4T = volume / 9.6, CAS/PCO = volume / 12.
    // PENTING: Point Trip TIDAK mengikuti bulan/quarter.
    // Quarter hanya menjadi penanda tahun. Semua Q1/Q2/Q3/Q4 pada tahun yang sama
    // menampilkan TOTAL POINT untuk SATU TAHUN tersebut.
    // Contoh: Q1 2026 = seluruh transaksi tahun 2026; Q2 2026 = seluruh transaksi tahun 2026.
    // Q1 2025 = seluruh transaksi tahun 2025; Q2 2025 = seluruh transaksi tahun 2025.
    // ================================================================
    var pointYearByOutlet = {};
    var pointTripMap = {};
    var sheetTripBWS = ss.getSheetByName("DATA TRIP BWS");
    if (sheetTripBWS && sheetTripBWS.getLastRow() >= 2) {
      var tripRows = sheetTripBWS.getDataRange().getValues();
      var tripHeaders = tripRows[0].map(function(h){ return String(h == null ? "" : h).trim().toUpperCase(); });
      var tripCatIdx = 0;
      var tripPointIdx = 1;
      for (var thx = 0; thx < tripHeaders.length; thx++) {
        if (tripHeaders[thx].indexOf("KAT") !== -1 || tripHeaders[thx].indexOf("PRODUK") !== -1 || tripHeaders[thx].indexOf("SKU") !== -1) tripCatIdx = thx;
        if (tripHeaders[thx].indexOf("POINT") !== -1 || tripHeaders[thx].indexOf("POIN") !== -1 || tripHeaders[thx].indexOf("VAL") !== -1) tripPointIdx = thx;
      }
      for (var tx = 1; tx < tripRows.length; tx++) {
        var tripCat = String(tripRows[tx][tripCatIdx] == null ? "" : tripRows[tx][tripCatIdx]).trim().toUpperCase();
        var tripPoint = Number(tripRows[tx][tripPointIdx]) || 0;
        if (tripCat) pointTripMap[tripCat] = tripPoint;
      }
    }

    for (var pi = 1; pi < dataDamen.length; pi++) {
      var prow = dataDamen[pi];
      var pStatus = cStatusCetak !== -1 ? String(prow[cStatusCetak] == null ? "" : prow[cStatusCetak]).trim().toUpperCase() : "";
      if (cStatusCetak !== -1 && pStatus !== "APPROVE") continue;

      if (!isAdmin) {
        var pCust = cCust !== -1 ? String(prow[cCust] == null ? "" : prow[cCust]).trim().toUpperCase() : "";
        if (!loginCustCode || pCust !== loginCustCode) continue;
      }

      var pOutlet = String(prow[cOutlet] == null ? "" : prow[cOutlet]).trim();
      if (!pOutlet) continue;
      var pDate = new Date(prow[cDate]);
      if (isNaN(pDate.getTime())) continue;

      var pYear = pDate.getFullYear();
      // Quarter transaksi hanya dipakai untuk menentukan channel TARGET OUTLET.
      // Point Trip sendiri diakumulasi berdasarkan TAHUN penuh.
      var pQuarter = "Q" + (Math.floor(pDate.getMonth() / 3) + 1) + " " + pYear;
      var pVolume = cVolume !== -1 ? (Number(prow[cVolume]) || 0) : 0;
      var pCategory = String(prow[39] == null ? "" : prow[39]).trim();
      if (!pCategory) pCategory = "Lainnya";
      var pCatUpper = pCategory.toUpperCase();
      var pRate = pointTripMap[pCatUpper] || 0;
      if (pRate === 0 && Object.keys(pointTripMap).length > 0) {
        for (var pk in pointTripMap) {
          if (pCatUpper.indexOf(pk) !== -1 || pk.indexOf(pCatUpper) !== -1) { pRate = pointTripMap[pk]; break; }
        }
      }

      var pCustNorm = cCust !== -1 ? String(prow[cCust] == null ? "" : prow[cCust]).trim().toLowerCase() : "";
      var pTargetKey = pOutlet.toUpperCase().replace(/\s+/g, " ").trim().toLowerCase() + "||" + pQuarter + "||" + pCustNorm;
      var pChannel = targetChannelMap[pTargetKey] || outletChannelMap[pOutlet] || "";
      var pChannelUpper = String(pChannel || "").trim().toUpperCase();
      var pDivisor = 0;
      if (pChannelUpper === "BP" || pChannelUpper === "MCO-4T") pDivisor = 9.6;
      else if (pChannelUpper === "CAS" || pChannelUpper === "PCO") pDivisor = 12;

      var pKarton = pDivisor > 0 ? pVolume / pDivisor : 0;
      var pPointValue = pKarton * pRate;
      var outletKeyPoint = pOutlet.toUpperCase().replace(/\s+/g, " ").trim();
      if (!pointYearByOutlet[outletKeyPoint]) pointYearByOutlet[outletKeyPoint] = {};
      var yearKeyPoint = String(pYear);
      if (!pointYearByOutlet[outletKeyPoint][yearKeyPoint]) {
        pointYearByOutlet[outletKeyPoint][yearKeyPoint] = {year:pYear, point:0, volume:0, karton:0, categories:{}};
      }
      var pAgg = pointYearByOutlet[outletKeyPoint][yearKeyPoint];
      pAgg.volume += pVolume;
      pAgg.karton += pKarton;
      pAgg.point += pPointValue;
      if (!pAgg.categories[pCategory]) pAgg.categories[pCategory] = { kategori:pCategory, volume:0, karton:0, pointRate:pRate, point:0, divisor:pDivisor };
      pAgg.categories[pCategory].volume += pVolume;
      pAgg.categories[pCategory].karton += pKarton;
      pAgg.categories[pCategory].point += pPointValue;
      if (!pAgg.categories[pCategory].pointRate && pRate) pAgg.categories[pCategory].pointRate = pRate;
      if (!pAgg.categories[pCategory].divisor && pDivisor) pAgg.categories[pCategory].divisor = pDivisor;
    }

    Object.keys(aggregated).forEach(function(qKey2) {
      Object.keys(aggregated[qKey2]).forEach(function(outletKey2) {
        var monItem = aggregated[qKey2][outletKey2];
        var normalizedOutlet = String(monItem.outlet || "").toUpperCase().replace(/\s+/g, " ").trim();
        var selectedYearForPoint = Number(String(qKey2).split(" ").pop()) || 0;
        var pq = (pointYearByOutlet[normalizedOutlet] && pointYearByOutlet[normalizedOutlet][String(selectedYearForPoint)]) || {year:selectedYearForPoint, point:0, volume:0, karton:0, categories:{}};
        monItem.pointYear = pq.year;
        // Nama field lama dipertahankan agar kompatibel dengan frontend.
        // Nilainya sekarang adalah TOTAL POINT TAHUNAN, bukan point kuartal.
        monItem.pointQuarter = qKey2;
        monItem.pointQuarterTotal = Number(pq.point.toFixed(2));
        monItem.pointQuarterVolume = Number(pq.volume.toFixed(2));
        monItem.pointQuarterKarton = Number(pq.karton.toFixed(2));
        monItem.pointByCategory = Object.keys(pq.categories).map(function(kc){
          var pc = pq.categories[kc];
          return {kategori:pc.kategori, volume:Number(pc.volume.toFixed(2)), karton:Number(pc.karton.toFixed(2)), pointRate:Number(pc.pointRate || 0), point:Number(pc.point.toFixed(2)), divisor:Number(pc.divisor || 0)};
...
function getProdukDibeliOutletQuarter(outletName, quarterKey, usernameInput) {
  try {
    var ss = SpreadsheetApp.getActiveSpreadsheet();
    // Detail Monitoring juga mengambil sumber dari DAMEN.
    // ORDERS hanya fallback bila DAMEN tidak tersedia.
    var sheet = ss.getSheetByName("DAMEN") || ss.getSheetByName("ORDERS");
    if (!sheet) return [];
    ...
    var isAdmin = String(usernameInput || "").trim().toLowerCase() === "admin";
    var loginCustCode = String(usernameInput || "").trim().toUpperCase();
    var idxCustCode = headers.indexOf("CUST.CODE");
    if (idxCustCode === -1) idxCustCode = headers.indexOf("CUST CODE");
    var resultList = [];
    ...
    for (var i = 1; i < data.length; i++) {
      ...
      // Untuk customer login, detail Monitoring tetap murni dari DAMEN/ORDERS
      // dan dibatasi oleh CUST.CODE yang sedang login.
      if (!isAdmin) {
        var rowCustCode = idxCustCode !== -1 ? String(row[idxCustCode] == null ? "" : row[idxCustCode]).trim().toUpperCase() : "";
        if (!loginCustCode || rowCustCode !== loginCustCode) continue;
      }
